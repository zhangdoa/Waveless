...
int main()
{
	...
	FrequencyBinArray X2Bin = l_math.getFreqBin(X2, l_sampleRate);
	...
	l_newWaveData.wavHeader = l_wavHeader;

	std::vector<short> l_newRawData;
	l_newRawData.reserve(l_rawDataProcessed.size());

	for (size_t i = 0; i < l_rawDataProcessed.size(); i++)
	{
		l_newRawData.emplace_back((short)l_rawDataProcessed[i].real());
	}

	l_newWaveData.rawData = l_newRawData;

	l_parser.writeFile("..//res//test_Processed.wav", l_newWaveData);
